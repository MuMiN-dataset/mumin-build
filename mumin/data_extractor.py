...
class DataExtractor:
    ...
    def extract_all(self,
        ...
        rels[rel] = self._extract_user_mentions_user(
            user_df=nodes['user']
        )
    ...
    def _extract_user_posted_tweet(self,
    ...
    def _extract_tweet_mentions_user(self,
        ...
        if self.include_mentions and mentions_exist:
            ...
            merged = (tweet_df[['entities.mentions']]
                      .dropna()
    ...
    def _extract_tweet_has_hashtag_hashtag(self,
        ...
        if self.include_hashtags and hashtags_exist:
            ...
            merged = (tweet_df[['entities.hashtags']]
                      .dropna()
                      .applymap(extract_hashtag)
                      .reset_index()
                      .rename(columns=dict(index='tweet_idx'))
                      .explode('entities.hashtags')
                      .merge(hashtag_df[['tag']]
                             .reset_index()
    ...
    def _extract_user_has_hashtag_hashtag(self,
        ...
        if self.include_hashtags and hashtags_exist:
            ...
            merged = (user_df[['entities.description.hashtags']]
                      .dropna()
                      .applymap(extract_hashtag)
                      .reset_index()
                      .rename(columns=dict(index='user_idx'))
    ...
    def _extract_user_has_url_url(self,
                                  user_df: pd.DataFrame,
                                  url_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if url_urls_exist or desc_urls_exist:
            ...
            if url_urls_exist:
                merged = (user_df[['entities.url.urls']]
                          .dropna()
                          .applymap(extract_url)
                          .reset_index()
                          .rename(columns=dict(index='user_idx'))
    ...
    def _extract_articles(self, url_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.include_articles:
            ...
            article_urls = [url for url in url_df.url.tolist()
    ...
    def _extract_images(self,
        ...
        if self.include_tweet_images or self.include_extra_images:
            ...
            image_urls = [url for url in url_df.url.tolist()
                          if url not in article_df.url.tolist()]
    ...
    def _extract_article_has_top_image_image(
        ...
        if self.include_articles and self.include_extra_images:
            ...
            img_idx = dict(index='im_idx')
            if self.include_articles:
                merged = (article_has_top_image_url_url_df
    ...
    def _extract_hashtags(self,
        ...
        if self.include_hashtags:
            ...
            if 'entities.hashtags' in tweet_df.columns:
                hashtags = (tweet_df['entities.hashtags']
                            .dropna()
                            .map(extract_hashtag)
                            .explode()
                            .tolist())
            ...
            if 'entities.description.hashtags' in user_df.columns:
                hashtags = (user_df['entities.description.hashtags']
                            .dropna()
                            .map(extract_hashtag)
                            .explode()
                            .tolist())
    ...
    def _extract_urls(self,
        ...
        if (self.include_extra_images and
                'profile_image_url' in user_df.columns):
    ...
    def _update_urls_from_articles(self,
                                   url_df: pd.DataFrame,
                                   article_df: pd.DataFrame) -> pd.DataFrame:
